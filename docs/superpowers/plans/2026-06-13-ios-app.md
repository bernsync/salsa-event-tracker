...
│   ├── AppModel.swift                   # Central @Observable state
│   │
│   ├── Services/
│   │   ├── SupabaseConfig.swift         # URL + anon key constants
│   │   ├── SupabaseService.swift        # All REST calls (events, trips, reviews)
│   │   └── AuthService.swift           # Login, logout, token refresh, keychain
│   │
│   ├── Models/
...
```bash
cd /private/tmp/salsa-ios-plan
git add ios/
git commit -m "feat(ios): scaffold Xcode project with SPM dependencies"
```
...
```bash
git add ios/SalsaEventTracker/Services/SupabaseService.swift
git commit -m "feat(ios): add SupabaseService REST client"
```
...
```bash
git add ios/SalsaEventTracker/Services/AuthService.swift
git commit -m "feat(ios): add AuthService with Keychain-backed session persistence"
```
...
- [ ] **Step 2: Update SalsaEventTrackerApp.swift**
...
```bash
git add ios/SalsaEventTracker/AppModel.swift ios/SalsaEventTracker/SalsaEventTrackerApp.swift
git commit -m "feat(ios): add AppModel with public/private data loading"
```
