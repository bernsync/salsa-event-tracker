...
### Supabase Anon Key — Safe to Embed and Commit

**The anon key is not a secret.** It is safe to hardcode in `SupabaseConfig.swift` and commit to git.

Supabase issues two keys per project:

| Key | Purpose | Safe in app? |
|-----|---------|--------------|
| **Anon key** (publishable) | Identifies the project; enforces RLS | **Yes** — embed freely |
| **Service role key** (secret) | Bypasses RLS entirely | **Never** — server-side only |

Anyone who decompiles the app binary can extract the anon key. This is fine by design — it's functionally equivalent to the key already visible in `web/supabase-config.js`. The security boundary is **Row Level Security on the database**, not key secrecy.

The service role key must never appear in the iOS app. It lives only in `scripts/` and in CI/CD environment variables.

### Verify RLS Is Enforced Before Connecting the App to Production

Confirm in the Supabase dashboard that RLS is correctly configured. This is a pre-flight check — do it before the app makes any authenticated calls.

**Public tables — anon role should be able to SELECT:**
- `events`
- `event_editions`
- `dance_styles`
- `schengen_countries`

**Private tables — anon role should have NO access; authenticated users only see their own rows:**
- `personal_trips` — policy: `owner_id = auth.uid()`
- `personal_trip_places` — policy: `owner_id = auth.uid()`
- `personal_pto_days` — policy: `owner_id = auth.uid()`
- `reviews` — policy: `user_id = auth.uid()`

**Test the anon role cannot read private data** (run this before shipping):
...
Expected response: `[]` (empty array, no error, no rows). If you see rows, RLS is misconfigured — stop and fix it before proceeding.

**Test that authenticated users can only see their own rows** by signing in as two different test accounts and confirming each only sees their own trips and reviews.

### Auth Token Storage

- Access tokens and refresh tokens are stored in the **iOS Keychain** via `KeychainAccess`, not `UserDefaults`. Keychain items are encrypted by the OS at rest and are isolated per app.
- Supabase access tokens expire after **1 hour** by default. `AuthService.restoreSession()` clears expired sessions on launch.
- **Token refresh is not implemented in this plan.** After 1 hour of use, API calls will return 401 and fail silently. Two options:
  - **Simple (implemented here):** Catch 401 in `SupabaseService`, sign the user out, and re-show the login sheet. Add this to `checkStatus`:
    ```swift
    // In SupabaseService.checkStatus, after the guard:
    if (response as? HTTPURLResponse)?.statusCode == 401 {
        throw AuthError.tokenExpired
    }
    ```
    Then in `AppModel`, catch `AuthError.tokenExpired` and call `signOut()`.
  - **Seamless (out of scope):** Call `POST /auth/v1/token?grant_type=refresh_token` with the stored refresh token before retrying.

### No Sensitive Data in Logs

- Never `print` or `os_log` auth tokens, refresh tokens, user IDs, or private row contents.
- `SupabaseService.checkStatus` surfaces the raw Supabase error body — that body may contain user context. Wrap any debug logging in `#if DEBUG`.
- In production builds, log only HTTP status codes, never response bodies for authenticated endpoints.

### Data in Transit

`URLSession` enforces TLS 1.2+ via App Transport Security (ATS) by default. Supabase uses HTTPS exclusively. **Do not add any `NSAppTransportSecurity` exceptions to `Info.plist`** — if the build requires one, that is a bug, not a configuration step.
...
| Concern | Decision |
|---------|----------|
| Certificate pinning | Not needed. Supabase runs on CDN infrastructure with rotating certs; pinning causes outages. TLS validation (iOS default) is sufficient. |
| Jailbreak detection | Not required. This is personal-use data (your own trips and reviews), not financial or health data. |
| Biometric auth gate | Not required for this iteration. Supabase session auth is sufficient. |
| OWASP Mobile Top 10 audit | Recommended before any public App Store release, not before initial development. |
