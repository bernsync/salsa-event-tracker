...
description: Find recent salsa festival editions from the Salsa Festival Tracker repo, check official sources for next editions, avoid duplicate event rows, and update or review weekly audit reports. Use when the user asks to refresh future editions, discover next editions from events since January 1 of the current year, process the weekly event-edition audit, or add newly announced salsa festival dates from official websites/social pages.
---
...
1. Read the latest GitHub issue or artifact generated by `Weekly next-edition discovery`.
